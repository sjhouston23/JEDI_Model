subroutine XRaySpectraDEAtm2(version,vAngle)
!*******************************************************************************
...
do i=1,11 !Skip the header of the file
  read(101,*)
end do
do Alt=1,AtmosLen
...
close(101)
!*** SULFUR
write(filename,"('./Output/Sul/',A,'/Photons_DE.dat')") trim(version)
open(unit=101,file=filename,status='old')
do i=1,11 !Skip the header of the file
  read(101,*)
end do
...
!*** OXYGEN
do OxyChS=6,7
  write(filename,"('./SyntheticSpectraTemplatesDE/O',I0,'.txt')") OxyChS
...
!*** Calculate synthetic spectra from oxygen precipitation
  do Eng=1,1 !Loop through every initial oxygen ion energy
    ...
    write(filename,"('./Output/XRayAtm2/',A,'/SpecVsAlt/Oxy_',I0,'_DE.dat')")&
          trim(version),OxyChS
    ...
    close(103)
    write(filename,"('./Output/XRayAtm2/',A,'/Points/Oxy_',I0,'_DE.dat')")&
          trim(version),OxyChS
...
!*** Calculate opacity effects
    do Ang=1,4 !Loop through each viewing angle
      ...
      call XRayOpacityAtm2(OxyXRaySpec,SynthSpec,nLines,vAngle(Ang),OxyOpacXRaySpec)
      write(filename,"('./Output/XRayAtm2/',A,'/Points/',I0,'/Oxy_',I0,&
                     '_DE.dat')") trim(version),int(vAngle(Ang)),OxyChS
      open(unit=106,file=filename)
      do i=1,nLines
...
!*** SULFUR
do SulChS=8,14
  write(filename,"('./SyntheticSpectraTemplatesDE/S',I0,'.txt')") SulChS
  open(unit=102,file=filename,status='old')
  read(102,*) !Skip header line
  nLines=0
  do i=1,nMaxLines
...
!*** Calculate synthetic spectra from sulfur precipitation
  do Eng=1,1
    ...
    write(filename,"('./Output/XRayAtm2/',A,'/SpecVsAlt/Sul_',I0,'_DE.dat')")&
          trim(version),SulChS
    open(unit=103,file=filename)
    ...
    close(103)
    write(filename,"('./Output/XRayAtm2/',A,'/Points/Sul_',I0,'_DE.dat')")&
          trim(version),SulChS
    open(unit=106,file=filename)
    do i=1,nLines
...
!*** Calculate opacity effects
    do Ang=1,4 !Loop through each viewing angle
      ...
      call XRayOpacityAtm2(SulXRaySpec,SynthSpec,nLines,vAngle(Ang),SulOpacXRaySpec)
      write(filename,"('./Output/XRayAtm2/',A,'/Points/',I0,'/Sul_',I0,&
                     '_DE.dat')") trim(version),int(vAngle(Ang)),SulChS
      open(unit=106,file=filename)
      do i=1,nLines
...
do Ang=1,4
  do Eng=1,1
    if(Ang.le.3)then
      write(filename,"('./Output/XRayAtm2/',A,'/Spectra/',I0,&
                     '_deg/O+S_Total_DE.dat')") trim(version),nint(vAngle(Ang))
    else
      write(filename,"('./Output/XRayAtm2/',A,&
                     '/Spectra/No_Opacity/O+S_Total_DE.dat')") trim(version)
    end if
...
1002 format(4x,F8.3,13x,ES9.3E2)
