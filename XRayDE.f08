subroutine XRaySpectraDE(version,vAngle)
!*******************************************************************************
...
!*** Calculate synthetic spectra from oxygen precipitation
  do Eng=1,1 !Loop through every initial oxygen ion energy
    ...
    write(filename,"('./Output/XRay/',A,'/SpecVsAlt/Oxy_',I0,'_DE.dat')")&
          trim(version),OxyChS
    ...
    close(103)
    write(filename,"('./Output/XRay/',A,'/Points/Oxy_',I0,'_DE.dat')")&
          trim(version),OxyChS
...
!*** Calculate opacity effects
    do Ang=1,4 !Loop through each viewing angle
      ...
      write(filename,"('./Output/XRay/',A,'/Points/',I0,'/Oxy_',I0,'_DE.dat')")&
...
!*** Calculate synthetic spectra from sulfur precipitation
  do Eng=1,1
    ...
    write(filename,"('./Output/XRay/',A,'/SpecVsAlt/Sul_',I0,'_DE.dat')")&
          trim(version),SulChS
    open(unit=103,file=filename)
    ...
    close(103)
    write(filename,"('./Output/XRay/',A,'/Points/Sul_',I0,'_DE.dat')")&
          trim(version),SulChS
    open(unit=106,file=filename)
    do i=1,nLines
...
!*** Calculate opacity effects
    do Ang=1,4 !Loop through each viewing angle
      ...
      write(filename,"('./Output/XRay/',A,'/Points/',I0,'/Sul_',I0,'_DE.dat')")&
