subroutine JEDIInterpolatorOxy(version,Iflux)
!*******************************************************************************
...
write(filename,'("./JunoData/Spectra/Oxy/",A,".d2s")') trim(version)
open(unit=100,file=trim(filename),status='old')
open(101,file='./Output/OxygenFlux.dat')
write(*,*)
...
write(101,1003)'Energy Bin:','JEDI Intensity:','Energy Bin Width:',&
               'Normalized Flux:' !Write out general information
...
!* ratio of oxygen:sulfur (from SO_2)
  ...
  write(101,1004)Jenergy(i),Jintensity(i),Jebins(i),Jflux(i)
end do
...
!These I decided by visually interpolating the data for the best fit.
Iintensity(nInterp)=Iintensity(nInterp)
Iintensity(nInterp+1)=Iintensity(nInterp)*5/7
Iintensity(nInterp+2)=Iintensity(nInterp)*4/7
...
!* publication concering JEDI would suggest it's more like 322.30.
  ...
  write(101,1004)Ienergy(i),Iintensity(i),Iebins(i),Iflux(i)
end do
write(*,*) sum(Iebins*Iintensity),sum(Jebins*Jintensity)
